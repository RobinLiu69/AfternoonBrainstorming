...
- **梯形(HF)-8/1-九宮格**/每次造成傷害則減少1點血且獲得1點攻擊，且此傷害不會直接使紅重裝死亡，而是延遲至回合結束。
> [!IMPORTANT]
> 其不會獲得分數。
...
- **梯形(HF)-8/2-九宮格**/破壞方塊會增加5%運氣值，並隨機位置放一個幸運方塊。
- **雙菱形(LF)-6/3-小十字**/破壞幸運方塊便對最近敵方單位造成傷害，且有25%返還刀。
- **刺客(ASS)-2/4-斜十字**/斬殺後獲得5%運氣值，且敵方運氣值減少5%。
- **六邊形(APT)-5/0-單體最近**/回合開始時，會將小十字內的空格放上幸運方塊，已方每次友方破壞幸運方塊獲得1點護盾。
...
- **三角形(ADC)-4/1-大十字**/利用藍球抽牌後會自動攻擊，若當下為麻痹效果則解除麻痹但不攻擊，展殺後獲得2點藍球。
- **圓形(AP)-3/2-單體最近**/攻擊附帶麻痺並獲得兩顆藍球。
...
- **方形(TANK)-9/1-小十字**/受到傷害刻印3層。
- **梯形(HF)-8/1-九宮格**/我方回合開始後受到2傷害並刻印2，造成傷害後恢復1點血，血量低於4時+2攻擊力。
...
- **圓形(AP)-2/1(6)-單體最近**/攻擊附帶麻痺並打劫3$，進場則攻擊單體最近2次(無視麻痺)/進場攻擊的目標改為由隨機友方單體攻擊，且可攻擊單位攻擊目標為本體最近(優先尋找除自己以外的單位)(+1/+2)。
...
- **三角形(ADC)-6/6-大十字**/攻擊後自身陷入麻痺。
- **圓形(AP)-5/9-單體最近**/攻擊使對手抽一張牌。
- **方形(TANK)-20/1-小十字**/若本回合沒有受到攻擊，則回合開始時-4生命。
- **梯形(HF)-11/5-九宮格**/每次攻擊消耗兩把刀，不足兩把則無法攻擊。
- **雙菱形(LF)-8/8-小十字**/斬殺使對手+2分。
- **刺客(ASS)-2/9-斜十字**/斬殺後自己下回合開始時不會自動抽牌。
- **六邊形(APT)-10/3-單體最近**/進場時所有對手+2護盾，攻擊使最近友方+1/+1，若其為巨人則再+1/+1。
- **鑽石(SP)-1/5-單體最遠**/攻擊進入狂暴 (詳見[效果介紹](#效果簡介))，並使己方巨人失去自身效果。
...
> [!IMPORTANT]
> 不被麻痹效果所無效化
- **刺客(ASS)-2/3-斜十字**/斬殺後抽(敵方場上堆-我方場上堆-2)張牌。
