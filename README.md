...
- **雙菱形(LF)-6/3-小十字**/攻擊後可移動，移動後對單體最近造成傷害。
...
- **圓形(AP)-3/1(6)-單體最近**/攻擊附帶麻痺並打劫3$，進場則攻擊單體最近2次(無視麻痺)/進場攻擊的目標改為由隨機友方單體攻擊，且可攻擊單位攻擊目標為本體最近(優先尋找除自己以外的單位)(+2+2)。
...
- **圓形(AP)-4/1-單體最近**/攻擊附帶麻痺，進場與自身回合開始時，對格子內的敵人附加麻痺效果。
