        ...
        match attack_type:
            ...
            case"nearest":
                ...
                if nearby_cards:
                    ...
                    for card in nearby_cards: yield card.board_x, card.board_y
...
def attack_area_display(controller: str, board_x: int, board_y: int, on_board_neutral: list[Card], player1_on_board: list[Card], player2_on_board: list[Card], board_dict: dict[str, Board], game_screen) -> None:
    target_cards = tuple(filter(lambda card: card.board_position == board_x+(board_y*4), on_board_neutral+player1_on_board+player2_on_board))
    if target_cards:
