...
class Player:
    ...
    def init_cards(self, game_screen: GameScreen) -> None:
        match self.name:
            case "player1":
                ...
                for i in range(3): self.draw_card(game_screen)
                game_screen.number_of_attacks[self.name] += 1
            case "player2":
                self.discard_pile = self.deck.copy()
                for i in range(3): self.draw_card(game_screen)
    
    def init_highlight_display(self, game_screen: GameScreen) -> None:
    ...
    def turn_end(self, game_screen: GameScreen) -> None:
        if game_screen.log: game_screen.log.write(f"{self.name} ended\n")
    ...
    def draw_card(self, game_screen: GameScreen) -> None:
        if self.draw_pile:
            ...
            if game_screen.log: game_screen.log.write(f"{self.name} drew {card}\n")
        else:
    ...
    def play_card(self, board_x: int, board_y: int, index: int, other_player: "Player", neutral: Neutral, board_dict: dict[str, Board], game_screen: GameScreen) -> None:
        ...
        match card:
            case "HEAL":
                ...
                if game_screen.log: game_screen.log.write(f"{self.name} used HEAL by playing {index}-{card}\n")
                self.discard_pile.append(self.hand.pop(index))
            case "MOVE":
                game_screen.number_of_movings[self.name] += 1
                if game_screen.log: game_screen.log.write(f"{self.name} used MOVE by playing {index}-{card}\n")
                self.discard_pile.append(self.hand.pop(index))
            case "MOVEO":
                game_screen.number_of_movings[self.name] += 1
                if game_screen.log: game_screen.log.write(f"{self.name} used MOVEO by playing {index}-{card}\n")
                self.hand.pop(index)
            case "CUBES":
    ...
    def recycle_cards(self, player1: Player, player2: Player, neutral: Neutral, board_dict: dict[str, Board], game_screen: GameScreen) -> None:
        for i, card in enumerate(self.on_board):
            if card.health <= 0 and card.can_be_killed(player1, player2, neutral, board_dict, game_screen):
                ...
                if game_screen.log: game_screen.log.write(f"{self.name} recycled {card_name}{card.board_x}-{card.board_y}\n")
