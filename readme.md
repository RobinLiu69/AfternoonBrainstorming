...
梯形(HF)-9/1-九宮格/紅重裝效果改為每次造成傷害則減少1點血且獲得1點攻擊 且此傷害不會直接使紅重裝死亡而是延遲至回合結束
...
圓形(AP)-3/2-單體最近/攻擊附帶麻痺和依幸運值隨機壞運效果 並依幸運值獲得隨機好運效果或無 (詳見[效果介紹](#效果簡介))

方形(TANK)-10/1-小十字/承受傷害後為敵方附加壞運效果或無 (詳見[效果介紹](#效果簡介))
...
刺客(ASS)-2/4-斜十字/斬殺後可移動並進入狂暴 (詳見[效果介紹](#效果簡介)) 狂暴狀態下斬殺會返回刀並解除狀態
